...
Note that we have substituted the `web3.eth.getGasPrice()` call in the legacy code with `web3.eth.getMaxPriorityFeePerGas()`. I won't bore you with the output, it looks the same. The [`eth_maxPriorityFeePerGas`](../../apis/ethereum/eth-maxpriorityfeepergas.md) method is documented [here](https://docs.alchemy.com/alchemy/documentation/apis/ethereum#eth\_maxpriorityfeepergas).
