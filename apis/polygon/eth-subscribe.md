---
description: Polygon API - Creates a new subscription for specified events.
---
...
| ----------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| [alchemy\_pendingTransactions](eth-subscribe.md#alchemy\_pendingtransactions) | Emits full transactions that are sent to the network, marked as "pending", and are sent from **** or to **** a certain address. |
| [newPendingTransactions](eth-subscribe.md#newpendingtransactions)             | Emits transaction hashes that are sent to the network and marked as "pending".                                                  |
| [newHeads](eth-subscribe.md#newheads)                                         | Emits new blocks that are added to the blockchain.                                                                              |
| [logs](eth-subscribe.md#logs)                                                 | Emits logs attached to a new block that match certain topic filters.                                                            |
...
* `hashesOnly` (optional): `boolean`&#x20;
  * Default value is `false`, where the response matches the payload of [eth\_getTransactionByHash](../ethereum/eth\_gettransactionbyhash.md#returns) . If set to `true`, the payload returned contains _only the hashes of the transactions_ that are added to the pending state, which matches the payload of [newPendingTransactions](https://docs.alchemy.com/alchemy/enhanced-apis/subscription-api-websockets#newpendingtransactions))
